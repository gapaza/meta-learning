...
class O1KnapsackC:
    ...
    def generate_items(self, n):
        ...
        fo_synergy_prob = 0.05
        low_synergy = np.random.uniform(0, 0.1, (n, n))
        high_synergy = np.random.uniform(50, 100, (n, n))
        synergy = np.where(np.random.rand(n, n) < fo_synergy_prob, high_synergy, low_synergy)

        # Make the synergy matrix symmetric
        synergy = np.triu(synergy)  # Take the upper triangle
        synergy += np.triu(synergy, 1).T  # Reflect to the lower triangle
        ...
        items = [[values[i], synergy[i], weight[i]] for i in range(n)]
...
if __name__ == '__main__':
    ...
    kp = O1KnapsackC(n=n, new_problem=True)
