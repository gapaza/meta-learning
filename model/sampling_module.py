...
class SamplingModule(decoding_module.DecodingModule, metaclass=abc.ABCMeta):
  ...
  def _grow_alive_seq(self,
    ...
    if self.enable_greedy:
      ...
      topk_ids_flt = tf.cast(topk_ids, dtype=tf.float32)
    ...
    if self.padded_decode:
      ...
      topk_seq = tf.concat([alive_seq, topk_ids_flt], axis=-1)
  ...
  def _get_new_finished_state(self, state: Dict[str, Any], new_seq: tf.Tensor,
    ...
    if not self.padded_decode:
      finished_seq = tf.concat(
          [tf.cast(finished_seq, dtype=tf.int32), tf.zeros([batch_size, 1], tf.int32)], axis=-1)
    ...
    finished_seq = tf.cast(finished_seq, dtype=tf.float32)
    intermed_seq = tf.multiply(new_seq, tf.cast(new_finished_flags, new_seq.dtype))
    intermed_seq = tf.cast(intermed_seq, dtype=tf.float32)
    finished_seq += intermed_seq
